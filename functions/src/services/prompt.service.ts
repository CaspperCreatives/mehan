...
export class PromptService {
    ...
    private promptRepository: PromptsRepository;

    constructor() {
        this.promptRepository = new PromptsRepository();
    }
    ...
    defaultPrompts = {
      ...
      the returing result must meet the ${SCORING_CRITERIA.find(criteria => criteria.section === 'summary')?.criteria} criteria
      ...
      the returing result must meet the ${SCORING_CRITERIA.find(criteria => criteria.section === 'experience')?.criteria} criteria
      ...
      the returing result must meet the ${SCORING_CRITERIA.find(criteria => criteria.section === 'experiences')?.criteria} criteria
      ...
      the returing result must meet the ${SCORING_CRITERIA.find(criteria => criteria.section === 'education')?.criteria} criteria
      ...
      the returing result must meet the ${SCORING_CRITERIA.find(criteria => criteria.section === 'skills')?.criteria} criteria
      ...
      the returing result must meet the ${SCORING_CRITERIA.find(criteria => criteria.section === 'headline')?.criteria} criteria
    ...
    async getSectionPrompt(section: string, language?: string): Promise<any> {
      const prompt = this.defaultPrompts[section as keyof typeof this.defaultPrompts];
    ...
    getCurrentUserProfileData() {
      ...
      return currentUser?.profileData || null;
    }
     ...
     */
    async getEnhancedHeadlinePrompt(content: string, language?: string): Promise<string> {
      ...
      const summary = profileData.summary || profileData.about?.content || '';
